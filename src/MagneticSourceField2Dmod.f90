...
!------------------------------------------------------------------------------
  ...
  ! new
  REAL(KIND=dp) :: hx, hy, x, y, curr
  REAL(KIND=dp), PARAMETER :: phi1 = -PI, phi2 = PI
  REAL(KIND=dp), PARAMETER :: dr = 0.7E-3, dz = 0.7E-3
  INTEGER :: nmax
  TYPE(Nodes_t) :: Nodes
  REAL(KIND=dp), ALLOCATABLE :: Hsx(:,:), Hsy(:,:)

  LOGICAL :: FirstVisit = .TRUE.
  SAVE FirstVisit, Hsx, Hsy
!------------------------------------------------------------------------------
  ...
  curr = GetConstReal( SolverParams, 'Operation Current', Found )
  IF (.NOT.FOUND) CALL Fatal('StaticMagneticField', 'The operation current is not defined.')

  nmax = GetInteger( SolverParams, 'Field Order', Found )
  IF (.NOT.FOUND) nmax = 5

  PRINT *, '- INPUTED OPERATION CURRENT:', curr, '[A]'
  PRINT *, '- MAXIMUM ORDER OF FIELD CALCULATION:', nmax
  
  IF (FirstVisit) THEN
    Active = GetNOFActive()
    ALLOCATE(Hsx(Active,10), Hsy(Active,10))
    ...
    DO t = 1, Active
      ...
      ! calculate field at each node
      DO i = 1, n
        x = Nodes % x(i)
        y = Nodes % y(i)

!      CALL GetArcConductorField( x, y, phi1, phi2, dr, dz, curr, hx, hy )
        CALL GetCircularSourceField( x, y, hx, hy )
!      CALL GetArcSourceField( x, y, phi1, phi2, curr, hx, hy )
!      CALL GetSourceField( x, y, curr, nmax, hx, hy )
  ...
  PRINT *, '- Length of Hs array: ', SIZE(Hsx,1), 'x', SIZE(Hsx,2)
...
!$omp parallel do private(Element,n,nd)   
    DO t=1,active
       ...
       ELSE
         CALL LocalMatrix(Element, n, nd, Hsx(t,1:n)*curr, Hsy(t,1:n)*curr)
       END IF
    END DO
!$omp end parallel do
...
!        END DO
      ...
      IF ( CSymmetry ) THEN
        ...
        FORCE(1:nd) = FORCE(1:nd) + IP % s(t) * DetJ * (LoadAtip * Basis(1:nd) - &
            (M_ip(1)+Hs_ip(1))*dBasisdx(1:nd,2)+(M_ip(2)+Hs_ip(2))*(dBasisdx(1:nd,1) + Basis(1:nd)/x))
      ELSE
        !FORCE(1:nd) = FORCE(1:nd) + IP % s(t) * DetJ * (LoadAtip * Basis(1:nd) + &
...
!------------------------------------------------------------------------------
  ...
  INTEGER :: nmax
!------------------------------------------------------------------------------
  INTEGER :: i, NWIRE, layer, turn
  REAL(KIND=dp), ALLOCATABLE :: xs(:), ys(:), polarity(:)
  REAL(KIND=dp) :: RC 

  LOGICAL :: FirstVisit = .TRUE.
  SAVE FirstVisit, xs, ys, polarity
!------------------------------------------------------------------------------

  ! do something only the first time the function is visited
  IF (FirstVisit) THEN
    OPEN(1,file='data/CoilAlignment.txt',status='old',action='read')

    READ(1,*) NWIRE, RC
    PRINT *, 'LOADING FILE CONTAINING WIRE ALIGNMENT.'
    PRINT *, 'NUMBER OF WIRES:', NWIRE 
    PRINT *, 'CENTER OF RADIUS:', RC, '[m]'
    ALLOCATE( xs(NWIRE), ys(NWIRE), polarity(NWIRE) )

    DO i = 1, NWIRE
      READ(1,*) layer, turn, xs(i), ys(i), polarity(i)
    END DO
  ...
  DO i = 1, NWIRE
    CALL GetFieldFromLineCurrent( xs(i)+RC, ys(i), x, y, curr, nmax, Hsx, Hsy )  
  END DO
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
  INTEGER :: i, NWIRE, layer, turn
  INTEGER, PARAMETER :: NGAUSS= 12
  
  REAL(KIND=dp), ALLOCATABLE :: xs(:), ys(:), polarity(:)
  REAL(KIND=dp) :: xi(NGAUSS), wi(NGAUSS)
  REAL(KIND=dp) :: ps, dhr1, dhz1, dhr2, dhz2, hr, hz, RC

  LOGICAL :: FirstVisit = .TRUE.
  SAVE FirstVisit, xs, ys, polarity, xi, wi
!------------------------------------------------------------------------------
  ...
  DO i = 1, NWIRE
    ...
    DO j=1, NGAUSS/2
      ...
      ! calculate magnetic field
      CALL GetFieldFromArcCurrent( xs(i)+RC, ps,  ys(i), x, y, dhr1, dhz1 ) 
      ! fliped wires to mirror the field 
      CALL GetFieldFromArcCurrent( xs(i)+RC, ps, -ys(i), x, y, dhr2, dhz2 ) 
...
!------------------------------------------------------------------------------
 SUBROUTINE GetCircularSourceField( r, z, Hsr, Hsz )
!------------------------------------------------------------------------------
! Calculate magetic field derived from a bundle of wires
!------------------------------------------------------------------------------
  USE DefUtils
  IMPLICIT NONE
  REAL(KIND=dp) :: r, z, Hsr, Hsz
!------------------------------------------------------------------------------
  INTEGER :: i, layer, turn, NWIRE 
  REAL(KIND=dp) :: RC
  
  REAL(KIND=dp), ALLOCATABLE :: rs(:), zs(:), polarity(:)
  REAL(KIND=dp) :: dhr, dhz, Iop, hr, hz

  LOGICAL :: FirstVisit = .TRUE.
  SAVE FirstVisit, rs, zs, polarity
!------------------------------------------------------------------------------
  ...
  IF (FirstVisit) THEN
    ...
    PRINT *, 'NUMBER OF WIRES:', NWIRE 
    PRINT *, 'CENTER OF RADIUS:', RC, '[m]'
    ALLOCATE( rs(NWIRE), zs(NWIRE), polarity(NWIRE) )

    DO i = 1, NWIRE
      READ(1,*) layer, turn, rs(i), zs(i), polarity(i)
    END DO
  ...
  DO i = 1, NWIRE
    ...
    ! y > 0
    CALL GetFieldFromCircularCurrent( rs(i)+RC,  zs(i), r, z, Iop, dhr, dhz )
    Hsr = Hsr + dhr
    Hsz = Hsz + dhz
  ...
  END DO
!------------------------------------------------------------------------------
 END SUBROUTINE GetCircularSourceField
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
 SUBROUTINE GetArcConductorField( xx, yy, phis1, phis2, dr, dz, curr, Hsx, Hsy )
!------------------------------------------------------------------------------
! Calculate magetic field derived from a bundle of wires
!------------------------------------------------------------------------------
  USE DefUtils
  IMPLICIT NONE
  REAL(KIND=dp) :: xx, yy, Hsx, Hsy, curr, phis1, phis2, dr, dz
!------------------------------------------------------------------------------
  INTEGER :: i, NWIRE, layer, turn 
  REAL(KIND=dp), ALLOCATABLE :: xs(:), ys(:), polarity(:)
  REAL(KIND=dp) :: Iop, rs, zs, RC
  REAL(KIND=dp) :: hr(4), hphi(4), hz(4)
  REAL(KIND=dp) :: Hrhat, Hphihat, Hzhat, r, z, curd
...
!------------------------------------------------------------------------------
  ...
  ! do something only the first time the function is visited
  IF (FirstVisit) THEN
    ! LOADING INFO OF WIRE COORDINATES
    OPEN(1,file='data/CoilAlignment.txt',status='old',action='read')
  ...
  DO i = 1, NWIRE
    ...
    ! CALCULATE FIELD INTEGRATION ALONG PHI DIRECTION
    CALL GetFieldFromArcConductor( rs+0.5*dr, phis1, phis2, zs+0.5*dz, r, 0.0d0, z, hr(1), hphi(1), hz(1) ) 
    CALL GetFieldFromArcConductor( rs-0.5*dr, phis1, phis2, zs-0.5*dz, r, 0.0d0, z, hr(2), hphi(2), hz(2) )
    CALL GetFieldFromArcConductor( rs+0.5*dr, phis1, phis2, zs-0.5*dz, r, 0.0d0, z, hr(3), hphi(3), hz(3) )
    CALL GetFieldFromArcConductor( rs-0.5*dr, phis1, phis2, zs+0.5*dz, r, 0.0d0, z, hr(4), hphi(4), hz(4) )

    Hrhat   = Hrhat   + hr  (1) + hr  (2) - hr  (3) - hr  (4)
    Hphihat = Hphihat + hphi(1) + hphi(2) - hphi(3) - hphi(4) 
    Hzhat   = Hzhat   + hz  (1) + hz  (2) - hz  (3) - hz  (4)

    ! FIELD DERIVED FROM FLIPED COIL
    CALL GetFieldFromArcConductor( rs+0.5*dr, phis1, phis2, -(zs+0.5*dz), r, 0.0d0, z, hr(1), hphi(1), hz(1) )
    CALL GetFieldFromArcConductor( rs-0.5*dr, phis1, phis2, -(zs-0.5*dz), r, 0.0d0, z, hr(2), hphi(2), hz(2) )
    CALL GetFieldFromArcConductor( rs+0.5*dr, phis1, phis2, -(zs-0.5*dz), r, 0.0d0, z, hr(3), hphi(3), hz(3) )
    CALL GetFieldFromArcConductor( rs-0.5*dr, phis1, phis2, -(zs+0.5*dz), r, 0.0d0, z, hr(4), hphi(4), hz(4) )
    ...
    Hsx = Hsx + Hrhat * curd / (4.0*PI)
    Hsy = Hsy + Hzhat * curd / (4.0*PI)
  END DO
!------------------------------------------------------------------------------
 END SUBROUTINE GetArcConductorField
!------------------------------------------------------------------------------

!------------------------------------------------------------------------------
 SUBROUTINE GetFieldFromArcConductor(rs, phis1, phis2, zs, r, phi, z, Hr, Hphi, Hz)
!------------------------------------------------------------------------------
! Calculate magnetic field strength, Hs, from a arc conductor
! Reference: urankar 
!
! rs, phis, zs: source point of line current in cylinderical coordinate
! r , phi , z : observe point
! Hr, Hphi, Hz: parameters of magnetic flux density
!
! Author: Ye Yang
! Date  : 02.01.2022
!------------------------------------------------------------------------------
    IMPLICIT NONE
    REAL(KIND=dp) :: rs, phis1, phis2, zs, r, z, phi, Hr, Hphi, Hz
!------------------------------------------------------------------------------
    INTEGER, PARAMETER :: NGAUSS = 12
    INTEGER :: j
    REAL(KIND=dp) :: gam, psi, Dfac, Bfac, Gfac, beta1, beta2, beta3
    REAL(KIND=dp) :: term, term1, phis
    REAL(KIND=dp) :: xi(NGAUSS), wi(NGAUSS)

    LOGICAL :: FirstVisit = .TRUE.
    SAVE FirstVisit, xi, wi
!------------------------------------------------------------------------------
    ...
    ! CALCULATE MAGNETIC FIELD
    DO j=1, NGAUSS
      phis = (phis2-phis1)*0.5*xi(j) + (phis1+phis2)*0.5

      ! PARAMETERS
      psi  = phis - phi
      Bfac = SQRT( rs**2 + r**2 - 2*r*rs*COS(psi) )
      Dfac = SQRT( gam**2 + Bfac**2 )
      Gfac = SQRT( gam**2 + (r*SIN(psi))**2 ) 
      beta1= (rs - r*COS(psi)) / Gfac
      beta2= gam / Bfac
      beta3= gam * (rs - r*COS(psi)) / (r*SIN(psi)*Dfac)

      term = Dfac + r*COS(psi)*ASINH(beta1)
      term1= gam*ASINH(beta1) - r*COS(psi)*ASINH(beta2) - r*SIN(psi)*ATAN(beta3) 

      ! INTEGRATION OF H
      Hr   = Hr   + COS(psi)*term * wi(j)
      Hphi = Hphi + SIN(psi)*term * wi(j)
      Hz   = Hz   + term1 * wi(j) 
    END DO

    Hr   = Hr   * (phis2 - phis1)*0.5 
    Hphi = Hphi * (phis2 - phis1)*0.5 
    Hz   = Hz   * (phis2 - phis1)*0.5 
!------------------------------------------------------------------------------
 END SUBROUTINE GetFieldFromArcConductor
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
 SUBROUTINE GetFieldFromCircularCurrent(rs, zs, r, z, curr, Hr, Hz)
!------------------------------------------------------------------------------
! Calculate magnetic field strength, Hs, from a circular current source
! Reference:
!
! rs, zs: source point of circular current
! r , z : observe point
! curr  : operation current
! Hr, Ht: magnetic field strength
!
! Author: Ye Yang
! Date  : 12.21.2021
!------------------------------------------------------------------------------
    ...
    REAL(KIND=dp) :: r, z, Hr, Hz, curr
    REAL(KIND=dp) :: rs, zs, k2, term, ellF, ellE
    INTEGER :: n
!------------------------------------------------------------------------------
    Hr = 0.0D0
    Hz = 0.0D0
    ...
    ! calculate magnetic field
    IF (ABS(rs-r).GT.1e-6.AND.ABS(zs-z).GT.1e-6) THEN
      k2   = 4.0*r*rs / ((r+rs)**2 + (z-zs)**2) 

      CALL ell_F(PI*0.5,SQRT(k2),ellF) 
      CALL ell_E(PI*0.5,SQRT(k2),ellE) 

      Hr   = -ellF + (rs**2+r**2+(z-zs)**2)*ellE/((rs-r)**2+(z-zs)**2)
      Hz   =  ellF + (rs**2-r**2-(z-zs)**2)*ellE/((rs-r)**2+(z-zs)**2)

      Hr   = Hr *curr*0.5 * (z-zs) / PI / r / SQRT((r+rs)**2 + (z-zs)**2)
      Hz   = Hz *curr*0.5 / PI / SQRT((r+rs)**2 + (z-zs)**2)

!      PRINT *, r,z,rs,zs,z-zs,(-ellE+(rs**2+r**2+(z-zs)**2)*ellF/((rs-r)**2+(z-zs)**2))*&
!              0.5*(z-zs) / PI / r / SQRT((r-rs)**2 + (z-zs)**2)
    END IF 
!------------------------------------------------------------------------------
 END SUBROUTINE GetFieldFromCircularCurrent
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
SUBROUTINE func_rf(x,y,z,rf)
!------------------------------------------------------------------------------
  REAL(KIND=dp) rf, x, y, z, ERRTOL, TINY, BIG, THIRD, C1, C2, C3, C4
  PARAMETER (ERRTOL=.08, TINY=1.5e-38, BIG=3.E37, THIRD=1.0/3.0)
  PARAMETER (C1=1.0/24.0, C2=.1, C3=3.0/44.0, C4=1.0/14.0)
...
!------------------------------------------------------------------------------
END SUBROUTINE func_rf
!------------------------------------------------------------------------------

!------------------------------------------------------------------------------
SUBROUTINE func_rd(x,y,z,rd)
!------------------------------------------------------------------------------
  REAL(KIND=dp) rd, x, y, z, ERRTOL, TINY, BIG, C1, C2, C3, C4, C5, C6
  PARAMETER (ERRTOL=.05,TINY=1.e-25,BIG=4.5E21,C1=3.0/14.0,C2=1.0/6.0,&
             C3=9.0/22.0,C4=3.0/26.0,C5=.25*C3,C6=1.5*C4)
  REAL(KIND=dp) alamb, ave, delx, dely, delz, ea, eb, ec, ed, ee, fac, sqrtx, sqrty,&
                sqrtz, sum, xt, yt, zt
...
1 CONTINUE
  ...
  ed = ea - 6.0*eb
  ee = ed + ec + ec
  rd = 3.0*sum + fac*(1.0 + ed*(-C1+C5*ed-C6*delz*ee)&
       + delz*(C2*ee+delz*(-C3*ec+delz*C4*ea))) / (ave*SQRT(ave))
!------------------------------------------------------------------------------
END SUBROUTINE func_rd
!------------------------------------------------------------------------------

! Legendre elliptic integral of the 1st kind
!------------------------------------------------------------------------------
SUBROUTINE ell_F(phi, ak, F)
!------------------------------------------------------------------------------
  REAL(KIND=dp) F, ak, phi
  REAL(KIND=dp) s, rf

  s = SIN(phi)
  CALL func_rf( COS(phi)**2, (1.0-s*ak)*(1.0+s*ak), 1.0D+0, rf )
  F = s*rf
!------------------------------------------------------------------------------
END SUBROUTINE ell_F
!------------------------------------------------------------------------------

! Legendre elliptic integral of the 2nd kind
!------------------------------------------------------------------------------
SUBROUTINE ell_E(phi, ak, E)
!------------------------------------------------------------------------------
  REAL(KIND=dp) E, ak, phi
  REAL(KIND=dp) cc, q, s, rf, rd

  s  = SIN(phi)
  cc = COS(phi)**2
  q  = (1.0-s*ak)*(1.0+s*ak)

  CALL func_rf(cc, q, 1.0D+0, rf)
  CALL func_rd(cc, q, 1.0D+0, rd)

  E  = s*(rf-((s*ak)**2)*rd/3.0)
!------------------------------------------------------------------------------
END SUBROUTINE ell_E
!------------------------------------------------------------------------------
